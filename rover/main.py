...
def bfs_distance(matrix, reconfiguration, rows, colums, start, dest):
    ...
    best_d = float("inf")
    queue = [[distance[start[0]][start[1]], start[0], start[1], -1, -1]]
    ...
    while queue:
        ...
        if y > 0 and yy != y-1 and reconfiguration[x][y-1] == reconfiguration[x][y] + reconfigure2(matrix[x][y], matrix[x][y-1]):
            if not visited[x][y-1]:
                visited[x][y-1] = True
                distance[x][y-1] = distance[x][y] + 1
                heappush(queue, [distance[x][y-1], x, y-1, x, y])
            elif distance[x][y-1] > distance[x][y] + 1:
                distance[x][y-1] = distance[x][y] + 1
                heappush(queue,[distance[x][y-1],x, y-1, x, y])
                
        if y+1 < colums and yy != y+1 and reconfiguration[x][y+1] == reconfiguration[x][y] + reconfigure2(matrix[x][y], matrix[x][y+1]):
            if not visited[x][y+1]:
                visited[x][y+1] = True
                distance[x][y+1] = distance[x][y] + 1
                heappush(queue,[distance[x][y+1],x, y+1, x, y])
        ...
        if x+1 < rows and xx != x+1 and reconfiguration[x+1][y] == reconfiguration[x][y] + reconfigure2(matrix[x][y], matrix[x+1][y]):
            if not visited[x+1][y]:
                visited[x+1][y] = True
                distance[x+1][y] = distance[x][y] + 1
                heappush(queue, [distance[x+1][y], x+1, y, x, y])
            elif distance[x+1][y] > distance[x][y] + 1:
                distance[x+1][y] = distance[x][y] + 1
                heappush(queue,[distance[x+1][y],x+1,y, x, y])

        if x > 0 and xx != x-1 and reconfiguration[x-1][y] == reconfiguration[x][y] + reconfigure2(matrix[x][y], matrix[x-1][y]):
            if not visited[x-1][y]:
                visited[x-1][y] = True
                distance[x-1][y] = distance[x][y] + 1
                heappush(queue,[distance[x-1][y],x-1, y, x, y])
